...
async def verify_token(authorization: Optional[str] = Header(None)) -> dict:
    ...
    try:
        ...
        return {
            ...
            "auth_provider": decoded_token.get("firebase", {}).get(
        ...
        try:
            ...
            if token_info.get("aud") != EXPECTED_CLIENT_ID:
                raise HTTPException(
                    status_code=401, detail="Token not issued for this application"
                )
...
async def get_current_user(token_data: dict = Depends(verify_token)) -> TokenData:
    """
    Extract user information from verified token.

    Args:
        token_data: Decoded token from verify_token
